...
for profile_url in data["profile_urls"]:
    ...
    # Assuming you're already in the loop that iterates over profile_urls

# Rest of your existing code for scraping profile details

# Find the education section
education_section = soup.find(lambda tag: tag.name == 'section' and tag.find('div', {'id': 'education'}))
if education_section:
    education_list = education_section.find('div', {'class': 'pvs-list__outer-container'})
    if education_list:
        ul_tag = education_list.find('ul')
        if ul_tag:
            li_tags = ul_tag.find_all('li', {'class': 'artdeco-list__item'})
            for li_tag in li_tags:
                school_name = li_tag.find('span', {'class': 't-bold'}).text.strip() if li_tag.find('span', {'class': 't-bold'}) else ''
     
                degree_name = li_tag.find('span', {'class': 't-14'}).text.strip() if li_tag.find('span', {'class': 't-14'}) else ''
              
                dates_attended = li_tag.find('span', {'class': 't-14', 'class': 't-normal'}).text.strip() if li_tag.find('span', {'class': 't-14', 'class': 't-normal'}) else ''
            
                education_text = f'{school_name}, {degree_name}, {dates_attended}'

                # Check if 'Education' key exists and append the new education entry
                if 'Education' in profiles_data[-1]:
                    profiles_data[-1]['Education'] += f'\n{education_text}'
                else:
                    profiles_data[-1]['Education'] = education_text
                print(education_text)
# Rest of your code for storing and exporting the data
